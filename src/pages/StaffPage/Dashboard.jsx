...
const Dashboard = () => {
  ...
  const renderItemDetails = (item) => {
    ...
    const documentFieldsConfig = {
      ...
      'event': ['document'],
      'event-attended': ['certificate_file'],
      'leave': ['document'],
      'online-course': ['certificate_file', 'certificates'],
      'achievement': ['certificate_file', 'memento_proof', 'cash_prize_proof']
    };

    const renderDocumentLink = (value) => {
      if (!value) return "Not Provided";
      
      const documentPath = value.startsWith('uploads/') 
        ? value 
        : `${baseUploadPath}${value}`;
                ...
                if (isSubDocField) {
                  return (
                    <div key={subKey} className="mb-1">
                      <p className="text-sm text-gray-700">
                        <span className="font-medium">{subDisplayKey}:</span> {renderDocumentLink(subValue)}
                      </p>
                    </div>
                  );
                }
