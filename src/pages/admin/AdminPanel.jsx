...
const AdminPanel = () => {
 ...
 const { students = [], batchWiseCounts, departmentWiseCounts} = useStudent();
  ...
  // Format file size to a readable format (e.g., KB, MB)
  ...
  const studentChartData = useMemo(() => {
    ...
    if (deptKeys.length === 0) return []; // Handle case when no departments exist
...
const handlePrevious = () => {
  ...
  const filteredUploadHistory = useMemo(() => {
    return bulkHistory.filter((item) =>
  ...
  // Toggle expanded row for upload history
  const toggleUploadRow = (id) => {
    setExpandedUploadId(expandedUploadId === id ? null : id);
  };

  // Toggle expanded row for download history
  const toggleDownloadRow = (id) => {
    setExpandedDownloadId(expandedDownloadId === id ? null : id);
  };
